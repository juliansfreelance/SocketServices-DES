...
bitset<64> DES::decrypt(const bitset<64>& plain, const bitset<64>& key){
	...
	for(int i = 0; i < 16; i++){
		nextLeft = right;
		right = left ^ f(right, subKeys[15 - i]); // La secuencia de programación de subclaves se invierte cuando se cifra
		left = nextLeft;
	}
...
bitset<64> DES::IP_1(const bitset<64>& bits){
	bitset<64> cipher;
	// Construir la permutación inversa de ip
	int ip_1[64];
	for(int i = 0; i < 64; i++)
		ip_1[ip[i] - 1] = i + 1;
	for(int i = 0; i < 64; i++)
...
bitset<32> DES::f(const bitset<32>& R, const bitset<48>& subKey){
	...
	// El tercer paso: dividido en 8 grupos, después del reemplazo de S-BOX 6-> 4
	for(int i = 0; i < 8; i++){
		int head = 47 - i * 6; // El primer índice de cada grupo
...
void DES::generateSubKeys(const bitset<64>& key){
	...
	// Obtenga 28 dígitos antes y después
	for (int i = 0; i < 28; ++i)
		left[i] = current[i + 28];
	for (int i = 0; i < 28; ++i)
		right[i] = current[i];
	...
	int shiftBits[] = {1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1}; // El número de dígitos desplazados a la izquierda
	for(int round = 0; round < 16; round++){
		...
		// Compresión y reemplazo para obtener una subclave de 48 bits
		for(int j = 0; j < 48; j++)
